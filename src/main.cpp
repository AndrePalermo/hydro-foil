...
if(decay){
	...
	string FD_file_sigma0 = output_folder + "/FeedDown_nointerpol_Sigma0";
	string FD_file_sigmastar = output_folder + "/FeedDown_nointerpol_Sigmastar";
	...
	for(double ipt : pT){
		for(double iphi : phi){
			// for(double iy : y_rap){
				Lambda_FeedDown_nointerpolation(ipt, iphi, 0, Sigma0, 
						hypersup, FDoutsigma0);
				Lambda_FeedDown_nointerpolation(ipt, iphi, 0, SigmaStar, 
						hypersup, FDoutsigmastar);
			}
		}
	}
