...
#define _REOMVE_TIMELIKES_ true
...
#pragma omp parallel for num_threads(threads_) reduction(+ : Denominator, P_vorticity, P_shear)
    ...
    {                                 // loop over the FO hypersurface
        ...
        for (int mu = 0; mu < 4; mu++)
        {
            ...
            for (int rh = 0; rh < 4; rh++)
                for (int sg = 0; sg < 3; sg++)
                { // since we are at midrapidity p_[3]=0 and we can stop at sg=2
                    // computing the shear induced polarization

                    for (int ta = 0; ta < 4; ta++)
                        P_shear[mu] += -pdSigma * nf * (1. - nf) * levi(mu, 0, rh, sg) * p_[sg] * p[ta] / p[0] * (cell.dbeta[rh][ta] + cell.dbeta[ta][rh]) / (8.0 * mass);
                }
        }
    }
            ...
            for (int nu = 0; nu < 4; nu++)
                for (int rh = 0; rh < 4; rh++)
                    for (int sg = 0; sg < 4; sg++)
                    {
                        ...
                        P_vorticity[mu] += pdSigma * nf * (1. - nf) * levi(mu, nu, rh, sg) * p_[sg] * projected_gradient[nu][rh];

                        if (nu == 0)
                            for (int ta = 0; ta < 4; ta++)
                                P_shear[mu] += -pdSigma * nf * (1. - nf) * levi(mu, nu, rh, sg) * p_[sg] * (p[ta] / p[0]) * t_vector[nu] * (projected_gradient[rh][ta] + projected_gradient[ta][rh]);
                    }
    }
            ...
            for (int nu = 0; nu < 4; nu++)
                for (int rh = 0; rh < 4; rh++)
                    for (int sg = 0; sg < 4; sg++)
                    {
                        ...
                        P_vorticity[mu] += pdSigma * nf * (1. - nf) * levi(mu, nu, rh, sg) * p_[sg] * cell.dbeta[nu][rh];

                        if (nu == 0)
                            for (int ta = 0; ta < 4; ta++)
                                P_shear[mu] += -pdSigma * nf * (1. - nf) * levi(mu, nu, rh, sg) * p_[sg] * (p[ta] / p[0]) * t_vector[nu] * (cell.dbeta[rh][ta] + cell.dbeta[ta][rh]);
                    }
    }
        ...
        for (int mu = 0; mu < 4; mu++)
        {
            ...
            for (int rh = 0; rh < 4; rh++)
                for (int sg = 0; sg < 4; sg++)
                    ...
                    {
                        P_shear[mu] += -pdSigma * nf * (1. - nf) * levi(mu, 0, rh, sg) * p_[sg] * p[ta] / p[0] * (cell.dbeta[rh][ta] + cell.dbeta[ta][rh]) / (8.0 * mass);
                        P_shear_chun[mu] += -pdSigma * nf * (1. - nf) * levi(mu, 0, rh, sg) * p_[sg] * p[ta] / pu * (cell.dbeta[rh][ta] + cell.dbeta[ta][rh]) / (8.0 * mass);
                    }
        }
    }
...
#pragma omp parallel for num_threads(threads_) reduction(+ : Denominator, polarization_mother, polarization_mother_shear)
            ...
            {                                 // loop over the FO hypersurface
                ...
                for (int mu = 0; mu < 4; mu++)
                {
                    for (int nu = 0; nu < 4; nu++)
                        for (int rh = 0; rh < 4; rh++)
                            for (int sg = 0; sg < 4; sg++)
                            {
                                theta_vector[mu] += levi(mu, nu, rh, sg) * P_mother_[sg] * cell.dbeta[nu][rh] / (2 * mother_mass);
                            }
                    theta_vector[mu] *= hbarC; // now theta is adimensional
                }
                ...
                double mutot = cell.mub * mother_baryonCharge + cell.muq * mother_electricCharge + cell.mus * mother_strangeness;
                ...
                for (int mu = 0; mu < 4; mu++)
                {
                    ...
                    for (int rh = 0; rh < 4; rh++)
                        for (int sg = 0; sg < 4; sg++)
                        {
                            // computing the shear induced polarization
                            for (int ta = 0; ta < 4; ta++)
                                polarization_mother_shear[mu] += -pdSigma * nf * (1. - nf) * levi(mu, 0, rh, sg) * P_mother_[sg] * P_mother[ta] / P_mother[0] * (cell.dbeta[rh][ta] + cell.dbeta[ta][rh]) / (8.0 * mass);
                        }
                }
            }
